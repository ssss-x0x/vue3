...
onErrorCaptured((event: DebuggerEvent): void => {
  console.log(`errorCaptured called: ${height.value} * ${width.value}`)
  console.log(event)
})

onActivated((): void => {
  console.log(`activated called: ${height.value} * ${width.value}`)
})

onDeactivated((): void => {
  console.log(`deactivated called: ${height.value} * ${width.value}`)
})
...
<template>
  <p>縦が {{ height }}, 横が {{ width }} の長方形は {{ area }}</p>
  <button @click="change">値を変更</button>
  <button @click="onError">エラーを出す</button>
</template>
